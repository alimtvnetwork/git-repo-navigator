...
`
	ScriptErrorPref = ""
	ScriptBanner    = `Write-Host ""
Write-Host "  ╔══════════════════════════════════════╗" -ForegroundColor DarkCyan
Write-Host "  ║          gitmap clone script         ║" -ForegroundColor DarkCyan
Write-Host "  ╚══════════════════════════════════════╝" -ForegroundColor DarkCyan
Write-Host ""
Write-Host "  Cloning %d repositories..." -ForegroundColor Cyan
Write-Host ""
`
	ScriptCounters = `$succeeded = 0
$failed = 0`
	ScriptRepoHeader = `Write-Host "  [%d/%d] " -ForegroundColor Magenta -NoNewline
Write-Host "%s" -ForegroundColor White
`
	ScriptMkdir = `$parentDir = Split-Path "%s" -Parent
if ($parentDir -and -not (Test-Path $parentDir)) {
    New-Item -ItemType Directory -Path $parentDir -Force | Out-Null
}
`
	ScriptCloneCmd = `$cloneResult = git clone -b %s %s "%s" 2>&1
`
	ScriptCloneCheck = `if ($LASTEXITCODE -eq 0) {
