...
All existing repo-level commands support a `--group` (`-g`) flag to
target an entire group:

| Command | Example |
|---------|---------|
| `pull` | `gitmap pull --group backend` |
| `exec` | `gitmap exec --group backend "git fetch --all"` |
| `status` | `gitmap status --group backend` |
| `release` | `gitmap release --group backend` |
| `clone` | `gitmap clone json --group backend` |

### `gitmap pull --all`

Pull all repos tracked in the database.
...
| `cmd/exec.go` | Add `--group` flag |
| `cmd/status.go` | Add `--group` flag |
| `cmd/scan.go` | Trigger DB upsert after scan |
| `model/record.go` | Add `Slug` field to `ScanRecord` |
| `spec/01-app/07-data-model.md` | Document new tables |
